# CUMCM 数学建模 Codex 技能套件

> 面向全国大学生数学建模竞赛（CUMCM / 国赛）**A、B、C 题**的赛时 Codex 技能（Skills）集合。
> 覆盖从“发题拆题 → 方法匹配 → 模型冻结 → 编码求解 → 结果复核 → 论文成稿 → 排版复核 → 终稿审计”到“赛后审查”的一站式工作流。

本仓库中的每个技能都是一个标准 Codex Skill 目录：`SKILL.md`（指令）、`references/`（内置知识）、`assets/`（模板与样式资产）、`scripts/`（自动化脚本）、`agents/`（Codex Agent 配置）。
...
| 技能目录 | 阶段 | 作用 | 常用触发说法 |
| --- | --- | --- | --- |
| `cumcm-live-problem-analyst` | ① 拆题 | 赛题刚发布时拆题、选题、附件盘点、问题依赖分析，输出“问题合同”并首轮分工；仅支持 A/B/C 题 | “刚发题先拆题”“分析 A/B/C 题”“列输入输出和阻断项”“形成赛时问题合同” |
...
| `visual-director` | 附加 | 中文社交图文/封面视觉方案（小红书 3:4、抖音 9:16、公众号封面等），与竞赛流程无关的可选技能 | “做图文方案”“视觉导演”“生成封面提示词” |
...
本套件不要求固定话术，直接“说人话”即可。以下触发说法与各技能 `SKILL.md` 中的场景一致：

| 你想做什么 | 直接这样说 |
| --- | --- |
| 刚发题，先拆题 | “比赛开始了，帮我分析 A 题，先拆题并形成问题合同。” |
| 给某小问找方法 | “Q2 属于什么类型？推荐 baseline 和候选模型。” |
| 冻结建模方案 | “根据问题合同冻结建模方案，写明假设、验证门和回退路线。” |
| Python 编码 | “用 Python 按冻结合同实现，固定 seed，输出结果表和论文图。” |
| MATLAB 编码 | “用 MATLAB 实现，先做工具箱预检。” |
| 复核结果 | “对冻结结果做独立复核，输出 VER 报告。” |
| 写论文 | “基于冻结结果写论文（LaTeX 路线），保留 AI 使用记录。” |
| 排版复核 | “对论文做排版复核，逐页检查渲染 PDF。” |
| 终稿审计 | “做提交前终稿审计，现在能提交吗？” |
| 赛后审查 | “对成稿论文做 9 维度深审和一致性检查。” |

建议在比赛工作目录中直接开启 Codex 会话（桌面版可把比赛文件夹作为工作区打开），并把官方题面与附件放入只读目录。
...
- **仅支持国赛 A、B、C 题**：遇到 D、E 题或其他竞赛，`cumcm-live-problem-analyst` 会输出 `BLOCKED_SCOPE` 并停止，不做“先按相似流程试试看”。
...
- 自研技能（`cumcm-live-*`、`cumcm-review`、`visual-director`）：版权归原作者所有，转载 / 分发请保留出处与作者信息（可按需补充 LICENSE 文件）。
